			...
			<c:choose>
				...
				<div class="span-3 column productResultsRightCol" style="float:right;text-align:right;">
					<span class="productPrice">
						<c:choose>
							<c:when test="${product.defaultSku.salePrice != null && product.defaultSku.salePrice != product.defaultSku.retailPrice }" >
								Sale: <span class="salePrice">$<c:out value="${product.defaultSku.salePrice}" /></span>
								<br/><span class="originalPrice">$<c:out value="${product.defaultSku.retailPrice}" /></span>
							</c:when>			
							<c:otherwise>
								<span class="salePrice">$<c:out value="${product.defaultSku.retailPrice}" /></span>
							</c:otherwise>
						</c:choose>
					</span> <br/><br/>
					<a class="addCartBtn" href="<c:url value="/basket/addItem.htm"> 
						<c:param name="skuId" value="${product.defaultSku.id}"/>
